    ...
    switch (message) {
        case WM_COMMAND:
			...
			return FALSE;
					...
					MessageBox(hWnd,
						"Warning: We are losing the handle to the default console screen buffer. As a result, you will not see the instructions at the lower half of the screen until we fix this. Until then, please use the non-GUI build to debug because that one works properly and is significantly faster. Read \"Help...About\" for more information. This GUI build is provided here as both a demo and as a template for a GUI gurus to improve.", 
						"Note:", MB_OK);
