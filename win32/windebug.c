...
void UpdateMemList()
{
	...
	else 
	{
		...
		{
			realmemloc = (uint8*)sDWORD_R[((memloc) >> 16)];
			if (realmemloc == NULL) 
			{
...
void RefreshOpList(char *opcode)
{
	...
	if (COP0Reg[COUNT] == NextClearCode)
	{
...
void UpdateCOP0()
{
	...
	for (i=0;i<32;i++)
	{
		if (COP0Reg[i] == 0)
			...
			sprintf(String, "%08X", (uint32)COP0Reg[i]);
...
void UpdateFPR()
{
	...
	for (i=0;i<32;i++)
	{
		...
		sprintf(String, "%016I64X", (_int64)COP1Reg[i]);
