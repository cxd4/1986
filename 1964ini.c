...
char * codecheck_type_names[] = {	"Default", "No Check", "Check DMA only", "Check DWORD", "Check QWORD", 
									"Check QWORD & DMA", "Check Block", "Check Block & DMA"};
...
// The array of ini entries. Array is sorted by Game Name
INI_ENTRY * ini_entries[MAX_INI_ENTRIES];

INI_ENTRY defaultoptions;		// 1964 default options
BOOL auto_run_rom = TRUE;
BOOL auto_apply_cheat_code = FALSE;
BOOL pause_at_inactive = FALSE;
BOOL pause_at_menu = FALSE;
BOOL use_default_save_directory = TRUE;
BOOL use_default_state_save_directory = TRUE;
BOOL use_default_plugin_directory = TRUE;
BOOL use_last_rom_directory = TRUE;
BOOL dma_in_segments = TRUE;
BOOL expert_user_mode = FALSE;
BOOL recent_rom_directory_list = FALSE;
BOOL recent_game_list = FALSE;
BOOL display_detail_status = TRUE;
BOOL state_selector_menu = FALSE;
BOOL critical_msg_window = FALSE;
BOOL display_romlist = TRUE;
...
void GenerateCurrentRomOptions(void)
{
	...
	if( RomListSelectedEntry()->pinientry->Code_Check == 0 )
	...
	if( RomListSelectedEntry()->pinientry->Emulator == 0 )
		currentromoptions.Emulator = defaultoptions.Emulator;

	if( RomListSelectedEntry()->pinientry->Max_FPS == 0 )
		currentromoptions.Max_FPS = defaultoptions.Max_FPS;
	
	if( RomListSelectedEntry()->pinientry->RDRAM_Size == 0 )
		currentromoptions.RDRAM_Size = defaultoptions.RDRAM_Size;
	
	if( RomListSelectedEntry()->pinientry->Save_Type == 0 )
		currentromoptions.Save_Type = defaultoptions.Save_Type;
	
	if( RomListSelectedEntry()->pinientry->Use_TLB == 0 )
	...
	if( RomListSelectedEntry()->pinientry->FPU_Hack == 0 )
...
int AddIniEntry(const INI_ENTRY *pnewentry)
{
	...
	if( ini_entry_count < MAX_INI_ENTRIES )
	{
		...
		while(	i<ini_entry_count )
		{
			...
			//DisplayError("i=%d, newcrc=%16X, crc=%16X, Compare Name=%d",i, newcrc,crc,compname);

			if( compname > 0 || ( compname == 0 && compcrc > 0 ) ||
		...
		{	
			...
			{	
				...
				int j;
				for( j=ini_entry_count; j>i; j--)
				ini_entries[j] = ini_entries[j-1];
			}
...
void DeleteIniEntry(const int index )
{
	...
	if( index < 0 || index >= ini_entry_count )
		return;
	...
	for( i=index; i<ini_entry_count-1; i++ )
		ini_entries[i] = ini_entries[i+1];
...
int	 FindIniEntry( const char * gamename, const uint32 crc1, const uint32 crc2, const uint8 countrycode)
{
	...
	while( i<ini_entry_count && (strcmp(ini_entries[i]->Game_Name, gamename) != 0 ||
	...
	if( i<ini_entry_count && strcmp(ini_entries[i]->Game_Name, gamename) == 0 &&
		ini_entries[i]->crc1 == crc1 &&
		ini_entries[i]->crc2 == crc2 &&
		ini_entries[i]->countrycode == countrycode )
	{
...
BOOL ReadIniEntry(FILE *pstream, INI_ENTRY *pnewentry)
{
	...
	if( feof(pstream) != 0 )	
		return FALSE;	// Read entry failed, reach to the end of file
	...
	do {
		if( strncmp(line,"[",1)==0 && strncmp(line+23,"]",1)==0 )
		{
			...
			//sscanf(line,"[%8X-%8X-C:%2X]",pnewentry->crc1, pnewentry->crc2, pnewentry->countrycode);
		}
		...
		else if(strncmp(line, "Check Self-modifying Code=", 26) == 0 )
		{
			...
			if( pnewentry->Code_Check > CODE_CHECK_MEMORY_BLOCK_AND_DMA )
				pnewentry->Code_Check = CODE_CHECK_MEMORY_QWORD;
		}
...
BOOL WriteIniEntry(FILE *pstream, const INI_ENTRY *p)
{
	...
	if( p->Code_Check != 0 && p->Code_Check != defaultoptions.Code_Check ) fprintf(pstream,"Check Self-modifying Code=%d\n",p->Code_Check);
	if( p->Emulator != 0 && p->Emulator != defaultoptions.Emulator ) fprintf(pstream,"Emulator=%d\n",p->Emulator);
	if( p->RDRAM_Size != 0 && p->RDRAM_Size != defaultoptions.RDRAM_Size ) fprintf(pstream,"RDRAM Size=%d\n",p->RDRAM_Size);
	if( p->Max_FPS != 0 && p->Max_FPS != defaultoptions.Max_FPS ) fprintf(pstream,"Max FPS=%d\n",p->Max_FPS);
	if( p->Save_Type != 0 && p->Save_Type != defaultoptions.Save_Type ) fprintf(pstream,"Save Type=%d\n",p->Save_Type);
	if( p->Use_TLB != 0 && p->Save_Type != defaultoptions.Use_TLB ) fprintf(pstream,"TLB=%d\n",p->Use_TLB);
	if( p->Eeprom_size != 0 && p->Eeprom_size != defaultoptions.Eeprom_size ) fprintf(pstream, "EEPROM Size=%d\n", p->Eeprom_size);
	if( p->Counter_Factor != 0 && p->Counter_Factor != defaultoptions.Counter_Factor ) fprintf(pstream,"Counter Factor=%d\n",p->Counter_Factor);
	if( p->Use_Register_Caching != 0 && p->Use_Register_Caching != defaultoptions.Use_Register_Caching ) fprintf(pstream, "Use Register Caching=%d\n", p->Use_Register_Caching);
	if( p->FPU_Hack != 0 && p->FPU_Hack != defaultoptions.FPU_Hack ) fprintf(pstream, "FPU Hack=%d\n", p->FPU_Hack);
	if( p->DMA_Segmentation != 0 && p->DMA_Segmentation != defaultoptions.DMA_Segmentation ) fprintf(pstream, "DMA=%d\n", p->DMA_Segmentation);
...
// I should not need to write such a stupid function to convert String to Init
// However, the scanf() function does not work for me to input hex number from
// inputted string. I spent some thing to debug it, not use, so to write
// this function to do the converting by myself.
