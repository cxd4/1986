...
BOOL ReadIniEntry(FILE *pstream, INI_ENTRY *pnewentry)
{
    ...
    do {
        if (strncmp(line, "[", 1) == 0 && strncmp(line + 23, "]", 1) == 0) {
            ...
            if (pnewentry->Code_Check > CODE_CHECK_PROTECT_MEMORY)
...
BOOL WriteIniEntry(FILE *pstream, const INI_ENTRY *p)
{
    ...
    if (p->FPU_Hack != 0 && p->FPU_Hack != defaultoptions.FPU_Hack)
    ...
    if (p->Use_HLE != 0 && p->Use_HLE != defaultoptions.Use_HLE)
